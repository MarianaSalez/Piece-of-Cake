...
async function getDietsRecipes(diet) {
 ...
 async function getInfoRecipe() {
        const recepies = await fetch(`https://api.spoonacular.com/recipes/{id}/information?apiKey=${API_KEY_RECIPE}&addRecipeInformation=true`)
        const infoLanding= recepies.data? recepies.map(elem=>{
            return {
                name:elem.title,
                img: elem.image,
