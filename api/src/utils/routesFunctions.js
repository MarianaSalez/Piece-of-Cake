...
const {API_KEY_ALL6, API_KEY_RECIPE6,API_KEY_DIET6} = process.env;
...
const getAllRecipes=async function() {
    const recepies = await axios(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY_ALL6}&addRecipeInformation=true&number=100`)
...
//Busqueda info detail
 ...
 async function getInfoRecipe(id) {
    if(Number(id)){
        const recipe = await axios(`https://api.spoonacular.com/recipes/${id}/information?apiKey=${API_KEY_RECIPE6}&addRecipeInformation=true`)
        if(recipe.data) {
            const detail= {
...
async function getDiets() {
    const recepies = await axios(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY_DIET6}&addRecipeInformation=true&number=100`)
   if (recepies.data.results) {
